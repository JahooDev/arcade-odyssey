...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "ARCADE.DEV — Junior Unity Developer Portfolio" },
      { name: "description", content: "An interactive retro-arcade portfolio for a junior Unity game developer. Play, unlock skills, and explore projects." },
      { property: "og:title", content: "ARCADE.DEV — Interactive Portfolio" },
      { property: "og:description", content: "Step into the arcade. Play mini-games to unlock real developer skills." },
    ],
  }),
  component: () => (
    <AppProvider>
  ...
  return (
    <div className="relative min-h-screen text-foreground">
      ...
      <header className="relative z-10 px-4 sm:px-8 py-4 flex items-center justify-between">
        <a href="#top" className="font-display text-sm text-glow-pink crt-flicker">▮ ARCADE.DEV</a>
        <nav className="hidden md:flex items-center gap-5 font-display text-[10px]">
          <a href="#arcade" className="text-glow-cyan hover:text-glow-pink transition">PLAY</a>
          <a href="#achievements" className="text-glow-cyan hover:text-glow-pink transition">{t.achievements.toUpperCase()}</a>
          <a href="#projects" className="text-glow-cyan hover:text-glow-pink transition">{t.projects.toUpperCase()}</a>
      ...
      <section id="top" className="relative px-4 pt-6 pb-16 text-center overflow-hidden">
        ...
        <div className="relative z-10 fade-up">
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center items-stretch max-w-2xl mx-auto">
            <button
              onClick={() => { start(); document.getElementById("arcade")?.scrollIntoView({ behavior: "smooth" }); }}
              className="neon-btn neon-btn-pink flex-1 group"
            >
              ▶ {t.arcadeMode}
              <div className="text-[8px] mt-1 opacity-70 normal-case tracking-normal">{t.arcadeModeDesc}</div>
            </button>
            <button
              onClick={() => { enableRecruiter(); document.getElementById("achievements")?.scrollIntoView({ behavior: "smooth" }); }}
              className="neon-btn flex-1"
            >
              ⚡ {t.recruiterMode}
              <div className="text-[8px] mt-1 opacity-70 normal-case tracking-normal">{t.recruiterModeDesc}</div>
            </button>
          </div>
          ...
          {PROJECTS.map((p) => (
            <article key={p.title} className="glass pixel-corners p-5 hover-lift">
              ...
              <div className="flex flex-wrap gap-1.5">
                {p.tech.map((tch) => (
                  <span key={tch} className="text-[10px] font-display px-2 py-1 border border-[var(--neon-cyan)]/40 text-glow-cyan rounded-sm">
                    {tch}
                  </span>
                ))}
              </div>
      ...
      <section id="contact" className="relative px-4 sm:px-8 py-16 max-w-2xl mx-auto text-center">
        ...
        <div className="glass pixel-corners p-6">
          ...
          <div className="flex gap-3 justify-center flex-wrap">
            <a href="mailto:hello@arcade.dev" className="neon-btn neon-btn-pink">✉ {t.sendEmail}</a>
            <a href="https://github.com" target="_blank" rel="noreferrer" className="neon-btn">⌨ {t.viewGithub}</a>
          </div>
        </div>
      </section>

      <footer className="text-center py-8 font-display text-[9px] text-muted-foreground">
        © {new Date().getFullYear()} ARCADE.DEV — <span className="text-glow-pink blink">PRESS START</span>
      </footer>
    </div>
  );
}
      ...
      {langs.map((l) => (
        <button key={l} onClick={() => onChange(l)}
          className={`font-display text-[10px] px-2 py-1 rounded-sm transition ${
            current === l ? "bg-[var(--neon-pink)] text-background" : "text-glow-cyan hover:text-glow-pink"
          }`}>
          {l.toUpperCase()}
        </button>
      ))}
